...
% figure
hold on
S1=stairs(t1,T1,'k','LineWidth',1); %Kaplan-Meier curve for treatment 1
if ~isempty(table12)
    S3=plot(xcg1,ycg1,'k+','LineWidth',1,'MarkerSize',5); %Censored data for treatment 1 (if there are)
else
    S3=[];
end
...
if ~isempty(table22)
    S4=plot(xcg2,ycg2,'r+','LineWidth',1,'MarkerSize',5); %Censored data for treatment 2 (if there are)
else
    S4 = [];
end
